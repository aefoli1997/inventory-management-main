...
export default {
  ...
  setup() {
    ...
    // Internally-submitted restock orders, surfaced in their own section
    const submittedOrders = computed(() => orders.value.filter(order => order.is_restock))
    ...
    // Derive lead time from the order's own dates so the column always reflects
    // the real expected_delivery span, not a hardcoded assumption.
